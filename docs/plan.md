...
7. **[HIGH]** Provider defaults corrected from vision:
   ...
   - Ollama: **`gemma4:*` is the primary default ladder** across every RAM tier where the user's Ollama install has Gemma 4 available; `qwen2.5-coder:7b` is the agent-mode alternative at 12 GB+; **Gemma 3 ladder is the documented fallback** only when Gemma 4 is unavailable in the user's Ollama version.
...
- **Updated provider defaults**: Anthropic `claude-opus-4-7` (no sampling params); OpenAI `gpt-5.4-mini`; **Ollama primary = Gemma 4 ladder by RAM** with `qwen2.5-coder:7b` for agent mode at 12 GB+ and Gemma 3 ladder kept only as a fallback for Ollama installs without Gemma 4. (r1 §7; r2 D5; **synthesis: maintainer direction 2026-04-18**)
...
- `biscuitcode-providers::ollama::OllamaProvider`:
  ...
  - **Primary default ladder = Gemma 4 by RAM (per maintainer direction):**
    | RAM | Primary default | Agent-mode alternative | Fallback if Gemma 4 unavailable |
    |---|---|---|---|
    | < 6 GB | `gemma4:e2b` | `gemma4:e2b` | `gemma3:1b` |
    | 6–12 GB | `gemma4:e2b` | `gemma4:e2b` | `gemma3:4b` |
    | 12–24 GB | `gemma4:9b` (or closest available Gemma 4 mid-tier) | `qwen2.5-coder:7b` (preferred for agent mode) | `gemma3:4b` + `qwen2.5-coder:7b` |
    | ≥ 32 GB | `gemma4:27b` (or closest available Gemma 4 large-tier) | `qwen2.5-coder:32b` | `gemma3:27b` |

    **Selection logic at first run:** ping `GET /api/tags` to list local models. If any `gemma4:*` is present, use it as the primary default per the table above. If not, attempt `ollama pull` of the smallest Gemma 4 tier appropriate to detected RAM; if that pull fails (e.g., older Ollama version that doesn't recognize the tag), fall back to the Gemma 3 ladder shown in the rightmost column and surface a one-time toast `Gemma 4 unavailable on your Ollama version; using Gemma 3. Update Ollama to access Gemma 4.` (catalogue code `E007 GemmaVersionFallback`).
...
- [ ] **After Ollama install on a 16 GB system: the picker shows `gemma4:9b` (or closest available Gemma 4 mid-tier) selected by default; `ollama list` confirms a Gemma 4 model was pulled.** On systems whose Ollama version does not recognize Gemma 4 tags, the picker shows a Gemma 3 default with the `E007 GemmaVersionFallback` toast and an "Update Ollama" link.
- [ ] On an 8 GB VM with Gemma 4 available: picker selects `gemma4:e2b`. On a 4 GB VM: picker selects `gemma4:e2b`. On systems without Gemma 4: 8 GB picks `gemma3:4b`, 4 GB picks `gemma3:1b`.
...
- [ ] **Gemma 4 default verified:** on a system with Gemma 4 available, the Ollama picker selects a Gemma 4 tier per the RAM table; on a system without Gemma 4, the `E007 GemmaVersionFallback` toast fires and the Gemma 3 ladder is selected.
...
16. **(Synthesis-added) Gemma 4 exact tag names.** Research-r2 verified `gemma4:e2b` (small multimodal). The Gemma 4 mid-tier (`gemma4:9b`) and large-tier (`gemma4:27b`) tag names in Phase 6a's RAM table are **extrapolated** from the Gemma 3 family naming convention — not verified against `https://ollama.com/library/gemma4`. The Phase 6a coder MUST verify these tag names at execution time and update the deliverable + AC if Ollama publishes Gemma 4 under a different naming convention (e.g., `gemma4:8b`, `gemma4:medium`, etc.). The runtime selection logic ("closest available Gemma 4 tier") tolerates drift, but the auto-pull command must match a real published tag.
