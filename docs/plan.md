...
19. **(Phase 7 → Phase 8 → Phase 9 coder; RESOLVED 2026-04-19)** **LSP hover/go-to-def frontend adapter — implemented without `monaco-languageclient`.** Previous coders deferred this due to `monaco-languageclient` v10.7.0 requiring 20+ `@codingame/monaco-vscode-api` peer packages that conflict with `vite-plugin-monaco-editor`. **Resolution (2026-04-19, attempt 1):** Implemented a thin, zero-new-dependency adapter using Monaco's public provider APIs (`registerHoverProvider`, `registerDefinitionProvider`) directly. `src/lsp/lspClient.ts` provides: (a) `LspClient` — JSON-RPC request/response correlator over Tauri `invoke('lsp_write')` + `listen('lsp-msg-in-<id>')`, with PM-01/PM-02/PM-03 mitigations built in; (b) `registerLspProviders()` — registers hover + definition providers on the Monaco global; (c) `useLspConnection` hook in `EditorArea.tsx` wires the client to the active tab's language/workspace. Build clean, 155 tests green, Vite bundle unchanged in size. Runtime integration with a live `rust-analyzer` is a smoke-test item for Mint 22 XFCE (requires rust-analyzer installed). Pre-mortem reconciliation: [PM-01] AVOIDED — response correlator checks `id !== undefined && (result || error)` before resolving; [PM-02] AVOIDED — `ensureSession()` is idempotent, module-level map prevents duplicate clients; [PM-03] AVOIDED — `monacoLangToLsp()` returns `null` for unsupported IDs, silently no-ops.
